...
bool displayFran::updateDisplay() {
	...
	for (int row = 0; row <= ROWMAX; row++) {
		for (int col = 0; col <= COLMAX; col++) {
			...
			al_draw_text(font, BLACK, POSX(col), POSY(row), ALLEGRO_ALIGN_CENTER, &c);
			al_flip_display();
		}
	}

	al_draw_filled_rectangle(POSX(cursor.column)-5, POSY(cursor.column), POSX(cursor.column) + RECT_WIDTH -5, POSY(cursor.row)+RECT_HEIGH+2, BLACK);
	al_flip_display();
	return	true;
}
